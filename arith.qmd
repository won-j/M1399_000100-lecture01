...
* Addition/subtraction: for unsigned integers, conducted according to usual binary arithmetic (carry/borrow); for signed integers, two's complement form is used and addition is carried out (carry only).
```
    57    00111001
-   35    11011101
-------------------
...
* Multiplication: for unsigned integers, left shift and additions; for signed integers, [Booth's algorithm](https://en.wikipedia.org/wiki/Booth%27s_multiplication_algorithm).

* Division: right shift and subtractions
...
#| vscode: {languageId: r}
#| output-fold: true
uint8::as.uint8(255) + uint8::as.uint8(1) # overflow
uint8::as.uint8(0) - uint8::as.uint8(1)  # underflow
```
...
* The term *floating-point number* refers to a real number that can be exactly represented in the format under discussion. 
    - They *represent* the real numbers closest to them (see rounding).
...
* In the floating-point number system, computer stores 
    - sign bit  
...
- Without gradual underflow, it may happen `x - y == 0` even though `x != y`.

```{r}
#| output-fold: true
x <- float::fl(1.234567e-38)  # close to single precision limit
y <- float::fl(1.234566e-38)
pryr::bits(float32bin(as.double(x)))
pryr::bits(float32bin(as.double(y)))  # x != y
pryr::bits(float32bin(as.double(x-y))) # x - y == 0 if normalized
```


### Rounding

* Given any fixed number of bits, most calculations with real numbers will produce quantities that cannot be exactly represented using that many bits. 
...
The IEEE 754 standard requires that the result of addition, subtraction, multiplication, division, square root, remainder, and conversion between integer and floating must be computed exactly then rounded to the nearest floating-point number (using round to even).

<!-- The IEEE 754 standard guarantees that the arithmetic operations involving floating point numbers is so that the operation result in the floating point representation of the exact arithmetric of the involved floating point numbers. -->

For example, suppose $x$ and $y$ are two exact real numbers. Let $[x]$ and $[y]$ be their floating point representation, after the appropriate rounding. Then, the addition of these two numbers is computed like the following.
$$
\begin{split}
...
1. Alignment: the exponents are compared and the fraction of the number having smaller expoenent is shifted right by a number of digits equal to the difference between the exponents.

2. Addition or subtraction of fractions takes place with the usual method.

3. The larger of the two exponents is taken as the resulting exponent.

4. Normalization: execute left shifts and exponents decreases until the leading digit is nonzero. Verify over/underflow.

5. Check the precision of the fraction. If over-precision, round it.
-->
...
Suppose we want to compute $x + y$, $x, y > 0$. Let the relative error of $x$ and $y$ be
$$
\delta_x = \frac{[x] - x}{x},
\quad
\delta_y = \frac{[y] - y}{y}
...
* Subtraction of two nearly equal numbers eliminates *all* significant digits.

* Occurs when the operands of subtraction are subject to rounding errors.

* Consider the quadratic discriminant formula $b^2 - 4ac$. Suppose decimal floating-point numbers with $p=3$. 
    -   $b=3.34$, $a=1.22$, $c=2.28$; $b^2 - 4ac = 0.0292$ (exact)
    -   $b^2 = 11.2$ (rounded), $4ac = 11.1$ (rounded); $b^2 - 4ac = 0.1$!

* The subtraction did not introduce any error but harmed by the errors introduced in the earlier multiplications.

* Another example: upper tail probabiilty of standard normal
```{r}
#| output-fold: true
...
- Rule 3: avoid substraction of two numbers that are nearly equal
    ...
    $$
            ...
            .
    $$
    If $b \approx \sqrt{b^2 - 4ac}$, then a catastrophic cancellation occurs. A resolution is to compute only one root $x_1 = \frac{-b - \sqrt{b^2 - 4ac}}{2a}$ by the formula, and find the other root $x_2$ using the relation $x_1x_2 = c/a$.

### Algebraic laws

Floating-point numbers may violate many algebraic laws we are familiar with, such as associative and distributive laws, as seen in the denormalization, machine epsilon, and floating-point arithmetic sections.

## Further readings

* Chapter 2, [Statistical Computing](https://books.google.co.kr/books/about/Statistical_Computing.html?id=i3uCDwAAQBAJ&redir_esc=y) by Kennedy and Gentle (1980).
...
* [Stat 3701 Lecture Notes](https://www.stat.umn.edu/geyer/3701/notes/arithmetic.html) by Charles J. Geyer (2020).